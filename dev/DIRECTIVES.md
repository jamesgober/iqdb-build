# iqdb-build -- Engineering Directives
...
iqdb-build orchestrates high-throughput index construction: split input into chunks, build sub-indexes in parallel, merge them. Loading a million vectors one at a time is slow; this is the bulk path. It is generic over the `Index` trait, so the same builder works for flat, HNSW, and IVF.
...
- `iqdb-index`: generic over any index
- `iqdb`: uses it for bulk ingestion
